...
std::string urlEncode(const std::string& input) {
  // Only the media paths generated by the packager itself flow through here
  // (user-supplied --base_urls are emitted verbatim by MpdBuilder). Per
  // RFC 3986, '/' separates path segments and must not be escaped, so escape
  ...
  while (true) {
    ...
    result += urlEncodeSegment(
        input.substr(start, slash == std::string::npos ? std::string::npos
                                                        : slash - start));
