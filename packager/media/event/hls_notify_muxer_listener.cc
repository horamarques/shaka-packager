    ...
    for (const auto& event_info : event_info_) {
      switch (event_info.type) {
        ...
        case EventInfoType::kCue:
          hls_notifier_->NotifyCueEvent(
              stream_id_.value(), event_info.cue_event_info.timestamp,
              event_info.cue_data, event_info.cue_event_info.is_cue_out,
              event_info.cue_event_info.duration_in_seconds);
          break;
      }
    }
    if (subsegment_index != num_subsegments) {
...
void HlsNotifyMuxerListener::OnCueEvent(int64_t timestamp,
  ...
  if (!media_info_->has_segment_template()) {
    ...
    event_info.cue_event_info = {timestamp, is_cue_out, duration_in_seconds};
    event_info.cue_data = cue_data;
    event_info_.push_back(event_info);
  } else {
    hls_notifier_->NotifyCueEvent(stream_id_.value(), timestamp, cue_data,
                                  is_cue_out, duration_in_seconds);
  }
}
